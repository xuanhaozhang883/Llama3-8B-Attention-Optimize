...
// Full-chain structural/numerical smoke test:
// raw Q/K -> RoPE -> QK -> causal mask -> Softmax -> TILE2 capture/repack
//         -> real TILE4 PV -> Context.
//
// This is intentionally a small deterministic test. It does not use the
// Python Golden Model yet. The standalone RoPE values, metadata, causal mask,
// stream counts and final Context values are all self-checked.
module tb_attention_system_with_rope_pv_small;
    ...
    localparam int RAW_REQUESTS_PER_GROUP =
        (Q_HEADS + 1) * SEQ_LEN * (HEAD_DIM/2);
    localparam int ROTATED_VECTORS_PER_GROUP =
        Q_HEADS * (SEQ_LEN/QK_TILE) *
        (SEQ_LEN/QK_TILE) * HEAD_DIM;
    ...
    integer raw_req_count;
    integer rotated_vec_count;
    integer ready_cycle;
    integer lane;
    integer g;
    integer h;
    ...
    // One-outstanding-request raw Q/K memory model. Every Q pair is (1,2)
    // and every K pair is (3,4). The small ROM is sin=1/cos=0, so RoPE must
    // produce Q=(-2,1) and K=(-4,3) in split-half layout.
    ...
    // Observe the actual vectors leaving the RoPE cache and entering QK.
    // These hierarchical names are local to the delivered wrapper and are
    // used only by this simulation testbench.
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            rotated_vec_count <= 0;
        end else if (dut.u_rope_bc_group.qk_vec_valid &&
            ...
            for (lane = 0; lane < QK_TILE; lane = lane + 1) begin
                if (dut.u_rope_bc_group.q_vec_bf16[lane*16 +: 16] !==
                    (($unsigned(req_dim) < HEAD_DIM/2) ?
                     16'hC000 : 16'h3F80))
                    $fatal(1,
                        "Rotated Q mismatch dim=%0d lane=%0d got=%h",
                        req_dim, lane,
                        dut.u_rope_bc_group.q_vec_bf16[lane*16 +: 16]);

                if (dut.u_rope_bc_group.k_vec_bf16[lane*16 +: 16] !==
                    (($unsigned(req_dim) < HEAD_DIM/2) ?
                     16'hC080 : 16'h4040))
                    $fatal(1,
                        "Rotated K mismatch dim=%0d lane=%0d got=%h",
                        req_dim, lane,
                        dut.u_rope_bc_group.k_vec_bf16[lane*16 +: 16]);
            end

            rotated_vec_count <= rotated_vec_count + 1;
        end
    end
    ...
    task automatic load_all_v_by_position;
        integer group_index;
                ...
                for (reduce_index = 0;
                    ...
                    for (feature_base = 0;
                        ...
                        for (load_lane = 0;
                            ...
                            // All features at reduce position k contain k+1.
                            // Uniform causal probabilities therefore produce
                            // Context rows 1.0, 1.5, 2.0 and 2.5.
                            case (reduce_index)
                                0: v_load_data[load_lane*16 +: 16] = 16'h3F80;
                                1: v_load_data[load_lane*16 +: 16] = 16'h4000;
                                2: v_load_data[load_lane*16 +: 16] = 16'h4040;
                                default:
                                   v_load_data[load_lane*16 +: 16] = 16'h4080;
                            endcase
                        end
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (context_valid && context_ready) begin
                ...
                // Constant Q/K scores produce uniform causal probabilities.
                // V[k]=k+1, so row r is average(1..r+1)=(r+2)/2.
                if (abs_real(
                        context_value - ((row_index + 2.0) / 2.0)
                    ) > 0.02)
                    $fatal(
                        1,
    ...
    initial begin
        ...
        if (raw_req_count !=
            GQA_GROUPS * RAW_REQUESTS_PER_GROUP)
            $fatal(1,
                "Raw request count mismatch expected=%0d got=%0d",
                GQA_GROUPS * RAW_REQUESTS_PER_GROUP,
                raw_req_count);

        if (rotated_vec_count !=
            GQA_GROUPS * ROTATED_VECTORS_PER_GROUP)
            $fatal(1,
                "Rotated QK vector count mismatch expected=%0d got=%0d",
                GQA_GROUPS * ROTATED_VECTORS_PER_GROUP,
                rotated_vec_count);
    ...
    initial begin
        #2000000000;
        $fatal(1, "TIMEOUT: raw-QK+RoPE+QK+Softmax+PV integration");
    end
