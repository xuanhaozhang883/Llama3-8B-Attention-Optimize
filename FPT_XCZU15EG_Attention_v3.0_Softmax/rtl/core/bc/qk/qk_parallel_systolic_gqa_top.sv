...
// Two-way (or one-way) QK tile scheduler.
//
...
module qk_parallel_systolic_gqa_top #(
    ...
    logic selected_last;
    logic selected_skip;
    logic [1:0] start_compute_count;
    logic [1:0] start_skip_count;
    ...
    always_comb begin
        ...
        start_skip_count = '0;
        for (lane = 0; lane < QK_LANES; lane = lane + 1) begin
            lane_active[lane] =
                ($unsigned(pair_col_base_reg) + lane*TILE) < SEQ_LEN;
            lane_skip[lane] =
                lane_active[lane] &&
                CAUSAL_TILE_SKIP &&
                causal_en &&
                (($unsigned(pair_col_base_reg) + lane*TILE) >
                 $unsigned(row_base_reg));
            lane_needs_vector[lane] =
                (state == S_RUN) &&
                lane_active[lane] &&
                !lane_skip[lane] &&
                tile_in_ready[lane] &&
                ($unsigned(lane_feed_count[lane]) < HEAD_DIM);
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            for (lane = 0; lane < QK_LANES; lane = lane + 1) begin
                lane_feed_count[lane] <= '0;
                skip_result_index[lane] <= '0;
            end
        end else begin
            done <= 1'b0;
            tile_start <= '0;
    ...
    initial begin
        if ((QK_LANES < 1) || (QK_LANES > 2))
            $error("qk_parallel_systolic_gqa_top: QK_LANES must be 1 or 2");
        if ((SEQ_LEN % TILE) != 0)
            $error("qk_parallel_systolic_gqa_top: TILE must divide SEQ_LEN");
        if (((SEQ_LEN/TILE) % QK_LANES) != 0)
            $error("qk_parallel_systolic_gqa_top: tile columns must divide by QK_LANES");
    end
endmodule
