...
#ifndef FPT_V26_EXPECT_OPTIMIZED
#define FPT_V26_EXPECT_OPTIMIZED 1
#endif

#define V26_QK_TILES_COMPUTED_EXPECTED       2112U
#define V26_QK_TILES_SKIPPED_EXPECTED        1984U
#define V26_MASKED_TILES_EMITTED_EXPECTED    1984U
#define V26_PV_TERMS_COMPUTED_EXPECTED    4227072U
#define V26_PV_TERMS_SKIPPED_EXPECTED      4161536U
#define V26_NATIVE_VECTORS_EXPECTED        4194304U
...
static uint32_t v26_profile_is_pass(const hw_profile_t *profile)
{
    if (profile->causal_error_flags != 0U)
        return 0U;
#if FPT_V26_EXPECT_OPTIMIZED
    return (
        profile->qk_tiles_computed ==
            V26_QK_TILES_COMPUTED_EXPECTED &&
        profile->qk_tiles_skipped ==
            V26_QK_TILES_SKIPPED_EXPECTED &&
        profile->masked_tiles_emitted ==
            V26_MASKED_TILES_EMITTED_EXPECTED &&
        profile->pv_reductions_computed ==
            V26_PV_TERMS_COMPUTED_EXPECTED &&
        profile->pv_reductions_skipped ==
            V26_PV_TERMS_SKIPPED_EXPECTED &&
        profile->native_vectors_captured ==
            V26_NATIVE_VECTORS_EXPECTED
    ) ? 1U : 0U;
#else
    return 1U;
#endif
...
#define PRINT_STAGE(label, field) do { \
    ...
    PRINT_STAGE("B+C/RoPE-QK-Softmax busy    ", bc_busy_cycles);
    PRINT_STAGE("Real PV busy                ", pv_busy_cycles);
    ...
    PRINT_STAGE("Repack input stall          ", repack_stall_cycles);
    PRINT_STAGE("Real-PV feed stall          ", pv_feed_stall_cycles);
    PRINT_STAGE("Softmax output stall        ", softmax_stall_cycles);
...
int main(void)
{
    ...
    xil_printf("\r\n================================================\r\n");
    xil_printf("FPT XCZU15EG Attention v2.6 Causal Dual-Tile benchmark\r\n");
    ...
    for (uint32_t warmup = 0U; warmup < WARMUP_RUNS; ++warmup) {
        ...
        if (status_is_pass(status) == 0U ||
            v26_profile_is_pass(&warmup_profile) == 0U ||
            compare_rc != 0) {
    ...
    for (uint32_t run = 0U; run < MEASURED_RUNS; ++run) {
        ...
        results[run].passed =
            (status_is_pass(results[run].status) != 0U &&
             v26_profile_is_pass(&results[run].profile) != 0U &&
             compare_rc == 0) ? 1U : 0U;
...
#undef AVG_FIELD
    ...
    xil_printf("[PASS] v2.4 fine-grained profiling and ten-run benchmark passed\r\n");
    return 0;
}
