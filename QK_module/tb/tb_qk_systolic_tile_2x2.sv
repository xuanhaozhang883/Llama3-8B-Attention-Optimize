...
module tb_qk_systolic_tile_2x2;
    ...
    initial begin
        ...
        // Feed Q rows 0/1 and K rows 0/1 for head 0.
        for (d = 0; d < HEAD_DIM; d = d + 1) begin
            q_rows_bf16[0*16 +: 16] = q_mem[0*FULL_SEQ*HEAD_DIM + 0*HEAD_DIM + d];
            q_rows_bf16[1*16 +: 16] = q_mem[0*FULL_SEQ*HEAD_DIM + 1*HEAD_DIM + d];
            k_cols_bf16[0*16 +: 16] = k_mem[0*HEAD_DIM + d];
            k_cols_bf16[1*16 +: 16] = k_mem[1*HEAD_DIM + d];
            in_valid = 1'b1;
        ...
        $display("========================================");
        $display("2x2 systolic tile test complete");
        $display("PASS = %0d", pass_count);
        $display("FAIL = %0d", fail_count);
        if ((pass_count == 4) && (fail_count == 0))
            $display("[PASS] 2x2 systolic tile");
        else
            $display("[FAIL] 2x2 systolic tile");
        $display("========================================");
    ...
    initial begin
        #10000000;
        $display("[TIMEOUT] 2x2 systolic tile did not finish");
        $finish;
    end
