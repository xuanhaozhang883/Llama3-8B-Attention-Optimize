...
本机已准备自包含目录 `D:\fpt_gui\v314`。连接板卡供电、JTAG 和底板 `USB3 / PS UART` 后，双击：

```text
D:\fpt_gui\v314\START_V314_BOARD.bat
```

脚本默认使用 `COM3`，依次完成四个产物 SHA-256 校验、只读 JTAG 探测、115200 8N1 UART 原始日志捕获、PS/DDR/UART 初始化、BIT/ELF 下载、等待 warm-up + 10-run 完成，以及正确性和性能签核。若串口号变化，可在命令行把新端口作为第一个参数，例如 `START_V314_BOARD.bat COM5`。未检测到 JTAG 或指定串口时脚本会在板卡写操作前停止。
