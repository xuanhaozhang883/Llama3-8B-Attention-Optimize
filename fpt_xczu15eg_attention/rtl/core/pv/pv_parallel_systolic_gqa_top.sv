...
module pv_parallel_systolic_gqa_top #(
    ...
    // Keep combinational and sequential loop indices separate. Vivado treats
    // a module-scope integer reused by always_comb and always_ff as a signal
    // with multiple procedural drivers during static elaboration.
    integer lane_comb;
    integer row_comb;
    integer lane_seq;
    integer row_seq;
    ...
    always_comb begin
        active_lane_count = 0;
        for (lane_comb = 0; lane_comb < PV_LANES;
             lane_comb = lane_comb + 1) begin
            lane_active[lane_comb] =
                ($unsigned(pair_col_base_reg) + lane_comb*TILE) < HEAD_DIM;
            if (lane_active[lane_comb])
                active_lane_count = active_lane_count + 1;
        end

        for (row_comb = 0; row_comb < TILE;
             row_comb = row_comb + 1) begin
            row_enable[row_comb] =
                !(CAUSAL_ROW_EFFECTIVE && causal_en) ||
                ($unsigned(reduce_reg) <=
                 ($unsigned(row_base_reg) + row_comb));
            row_first[row_comb] =
                row_enable[row_comb] && ($unsigned(reduce_reg) == 0);
            row_last[row_comb] =
                row_enable[row_comb] &&
                ((CAUSAL_ROW_EFFECTIVE && causal_en) ?
                    ($unsigned(reduce_reg) ==
        ...
        for (lane_comb = 0; lane_comb < PV_LANES;
             lane_comb = lane_comb + 1) begin
            if (lane_active[lane_comb] && !tile_in_ready[lane_comb])
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_START: begin
                    ...
                    for (lane_seq = 0; lane_seq < PV_LANES;
                         lane_seq = lane_seq + 1) begin
                        if (lane_active[lane_seq])
                            tile_start[lane_seq] <= 1'b1;
                    end
                    state <= S_FEED;
                end
                ...
                S_FEED: begin
                    if (vec_valid && vec_ready) begin
                        if (CAUSAL_ROW_EFFECTIVE && causal_en) begin
                            for (row_seq = 0; row_seq < TILE;
                                 row_seq = row_seq + 1) begin
                                if (!row_enable[row_seq] &&
                                    (p_vec_bf16[row_seq*16 +: 16] != 16'h0000))
