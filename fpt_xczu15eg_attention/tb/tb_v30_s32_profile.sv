...
// S32/D128 quick pre-optimization qualification for v3.0 Online-Fused Attention.
// This simulation-only code does not alter or wrap the design math.
module tb_v30_s32_profile;
    localparam int SEQ = 32;
    localparam int SOURCE_SEQ = 128;
    localparam int DIM = 128;
    localparam int GROUPS = 8;
    localparam int Q_PER_GROUP = 4;
    localparam int Q_HEADS = GROUPS*Q_PER_GROUP;
    localparam int Q_WORDS = Q_HEADS*SOURCE_SEQ*DIM;
    localparam int KV_WORDS = GROUPS*SOURCE_SEQ*DIM;
    localparam int PROFILE_KV_WORDS = GROUPS*SEQ*DIM;
    localparam int CONTEXT_WORDS = Q_HEADS*SEQ*DIM;
    localparam int RAW_REQUESTS = GROUPS*(Q_PER_GROUP+1)*SEQ*(DIM/2);
    localparam int ROW_TILES = SEQ/4;
    localparam int TILES_PROCESSED = Q_HEADS*ROW_TILES*(ROW_TILES+1)/2;
    localparam int TILES_SKIPPED = Q_HEADS*ROW_TILES*ROW_TILES-TILES_PROCESSED;
    localparam int V_VECTORS = TILES_PROCESSED*DIM;
    localparam int MAC_TERMS = Q_HEADS*SEQ*(SEQ+1)/2*DIM;
...
`endif
    ...
    logic [15:0] golden_mem [0:Q_WORDS-1];
    logic context_seen [0:CONTEXT_WORDS-1];
    ...
    always @(posedge clk) begin : RAW_MEMORY_MODEL
        ...
        if (!rst_n) begin
            ...
            if (raw_req_valid && raw_req_ready) begin
                base = $unsigned(raw_req_head)*SOURCE_SEQ*DIM +
    ...
    always @(posedge clk) begin : CHECK_AND_PROFILE
        ...
        if (!rst_n) begin
            ...
            if (group_complete) begin
                ...
                $display("[PROGRESS] group %0d/8 complete cycles=%0d outputs=%0d",
                         group_complete_count, total_cycles, output_count);
            end
            if (done) profiling = 1'b0;

            if (context_valid && context_ready) begin
                expected_index = $unsigned(context_global_q_head)*SOURCE_SEQ*DIM +
                                 $unsigned(context_row)*DIM +
                                 $unsigned(context_col);
                compact_index = $unsigned(context_global_q_head)*SEQ*DIM +
                                $unsigned(context_row)*DIM +
                                $unsigned(context_col);
                ...
                if (context_seen[compact_index]) failures = failures + 1;
                context_seen[compact_index] = 1'b1;
                if (context_group_last !==
                    (($unsigned(context_head) == Q_PER_GROUP-1) &&
                     ($unsigned(context_row) == SEQ-1) &&
                     ($unsigned(context_col) == DIM-1)))
                    failures = failures + 1;
                if (context_global_last !==
                    (($unsigned(context_group_id) == GROUPS-1) &&
                     ($unsigned(context_head) == Q_PER_GROUP-1) &&
                     ($unsigned(context_row) == SEQ-1) &&
                     ($unsigned(context_col) == DIM-1)))
                    failures = failures + 1;
    ...
    task automatic load_all_v;
        integer address;
        integer group_index;
        integer within_group;
        integer source_address;
        begin
            for (address = 0; address < PROFILE_KV_WORDS; address = address + 4) begin
                group_index = address/(SEQ*DIM);
                within_group = address%(SEQ*DIM);
                source_address = group_index*SOURCE_SEQ*DIM + within_group;
                @(negedge clk);
                v_load_addr = address[14:0];
                v_load_data = {v_mem[source_address+3], v_mem[source_address+2],
                               v_mem[source_address+1], v_mem[source_address]};
                v_load_valid = 1'b1;
    ...
    initial begin : TEST
        ...
        integer index;
        for (index = 0; index < CONTEXT_WORDS; index = index+1)
            context_seen[index] = 1'b0;
        $readmemh(Q_FILE, q_mem);
        $readmemh(K_FILE, k_mem);
        $readmemh(V_FILE, v_mem);
        $readmemh(GOLDEN_FILE, golden_mem);
        actual_fd = $fopen("v30_s32_actual_bf16.hex", "w");
        ...
        for (index = 0; index < CONTEXT_WORDS; index = index+1)
            if (!context_seen[index]) final_failures = final_failures + 1;
        ...
        if (qk_tiles_computed != TILES_PROCESSED ||
            qk_tiles_skipped != TILES_SKIPPED ||
            masked_tiles_emitted != TILES_SKIPPED)
            final_failures = final_failures + 1;
        if (online_tiles_processed != TILES_PROCESSED ||
            online_tiles_skipped != TILES_SKIPPED ||
            online_v_vectors_read != V_VECTORS ||
            online_mac_terms != MAC_TERMS) final_failures = final_failures + 1;
        if (start_while_busy_error || qk_causal_skip_error || v_cache_error ||
            online_order_error || online_numeric_error || protocol_error)
            final_failures = final_failures + 1;

        profile_fd = $fopen("v30_s32_profile.csv", "w");
        ...
        $display("[CONTRACT] raw=%0d/%0d groups=%0d/%0d qk=%0d/%0d skip=%0d/%0d",
                 raw_request_count, RAW_REQUESTS,
                 group_complete_count, GROUPS,
                 qk_tiles_computed, TILES_PROCESSED,
                 qk_tiles_skipped, TILES_SKIPPED);
        $display("[CONTRACT] online=%0d/%0d skipped=%0d/%0d v=%0d/%0d mac=%0d/%0d",
                 online_tiles_processed, TILES_PROCESSED,
                 online_tiles_skipped, TILES_SKIPPED,
                 online_v_vectors_read, V_VECTORS,
                 online_mac_terms, MAC_TERMS);
        if (final_failures == 0) begin
            marker_fd = $fopen("v30_s32_profile_pass.txt", "w");
            $fdisplay(marker_fd, "V3.0 S32/D128 8-GQA PROFILE: SIM_PASS");
            $fclose(marker_fd);
            $display("[PASS] V3.0 S32/D128 8-GQA QUICK PROFILE");
        end else begin
