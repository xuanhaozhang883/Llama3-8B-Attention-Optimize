...
puts $fh {set_property HD.CLK_SRC BUFGCE_X0Y0 [get_ports axi_clk]}
...
set axi_in {axi_rst_n wr_valid wr_kind wr_buffer wr_epoch wr_group wr_global_q_head wr_row_window wr_beat_index wr_data wr_strb wr_last done_ready}
...
set core_in {core_rst_n active_valid active_buffer q_req_valid q_req_context_tag q_req_d k_req_valid k_req_context_tag k_req_key_block k_req_d v_req_valid v_req_context_tag v_req_key v_req_feature_block}
...
if {[llength $paths] == 0} {
    error "CATS_R4_QKV_AXI_BANK_BRIDGE_OOC: no constrained timing path returned"
}
set worst_slack [get_property SLACK [lindex $paths 0]]
puts "CATS_R4_QKV_AXI_BANK_BRIDGE_OOC: worst_slack=$worst_slack"
if {$worst_slack < 0.0} {
    error "CATS_R4_QKV_AXI_BANK_BRIDGE_OOC: timing failed, worst_slack=$worst_slack"
}
close_project
