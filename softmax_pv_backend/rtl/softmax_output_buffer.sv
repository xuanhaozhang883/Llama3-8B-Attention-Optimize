...
module softmax_output_buffer #(
    ...
    // The bank bit is folded into the address so both ping-pong banks can use
    // one simple dual-port RAM. For TILE=2 and SEQ_LEN=128 this is 8 Kibit and
    // fits in one RAMB18 on Zynq UltraScale+.
    (* ram_style = "block" *) logic [TILE*16-1:0] p_mem [0:P_DEPTH-1];
    ...
    logic [TILE*16-1:0] p_rsp_data_reg;
    logic run_active;
    ...
    assign p_rsp_data  = p_rsp_data_reg;
    ...
    // Independent write and registered read ports form the BRAM inference
    // template. Memory contents do not need reset; bank_full protects them.
    always_ff @(posedge clk) begin
        if (!rst_n || start) begin
            p_rsp_data_reg <= '0;
        end else begin
            if (s_valid && s_ready)
                p_mem[wr_mem_addr][wr_local_row*16 +: 16] <= s_data;

            if (p_req_valid && p_req_ready) begin
                if ($unsigned(p_req_reduce_index) >= SEQ_LEN)
                    p_rsp_data_reg <= '0;
                else
                    p_rsp_data_reg <= p_mem[rd_mem_addr];
            end
        end
    end
