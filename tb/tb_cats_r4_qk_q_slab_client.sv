...
module tb_cats_r4_qk_q_slab_client;
    ...
    task automatic send_done(input logic [1:0] key_block);
        begin
            @(negedge clk);
            engine_done_valid = 1;
    ...
    initial begin
        ...
        for (kb = 0; kb < 4; kb = kb + 1) begin
            repeat (2) begin
                ...
                if (!engine_start_valid || engine_start_key_block !== kb ||
                    ...
                    $fatal(1, "engine start changed under backpressure key_block=%0d", kb);
            end
        ...
        if (jobs_accepted !== 1 || q_slab_needs_transferred !== 1 ||
            q_slab_ready_transferred !== 1 || engine_jobs_started !== 4 ||
            engine_jobs_completed !== 4 || q_slab_retires_transferred !== 1 ||
