...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
    $BuildDir = Join-Path $env:TEMP "fpt_v31_flash_consumer"
}
...
try {
    foreach ($Test in $Tests) {
        $Image = Join-Path $BuildDir "$($Test.Name).vvp"
        & iverilog -g2012 -Wall -s $Test.Name -o $Image @($Test.Sources)
        if ($LASTEXITCODE -ne 0) { throw "Compilation failed for $($Test.Name)" }
        $Output = & vvp $Image 2>&1
        $ExitCode = $LASTEXITCODE
        $Output | Write-Host
        if (($ExitCode -ne 0) -or (($Output -join "`n") -notmatch $Test.Pass)) {
            throw "Regression failed for $($Test.Name)"
        }
    }
} finally {
    Pop-Location
...
Write-Host "[PASS] v3.1 FlashAttention consumer checks passed."
